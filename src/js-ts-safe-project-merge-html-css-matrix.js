const HtmlCssProjectMergeMissingSignals = Object.freeze({
  ...
  svgParserEvidence: 'svg-parser-source-evidence-missing',
  svgReferenceGraphEvidence: 'svg-reference-graph-evidence-missing',
  htmlIdentityEvidence: 'html-identity-evidence-missing',
  svgIdentityEvidence: 'svg-identity-evidence-missing',
  cssSelectorTargetEvidence: 'css-selector-target-evidence-missing',
  htmlStructuralMerge: 'html-structural-merge-proof-blocked',
  svgStructuralMerge: 'svg-structural-merge-proof-blocked',
  ...
  svgBrowserRuntimeProof: 'svg-browser-runtime-proof-not-available',
...
function htmlCssProjectMergeMissingEvidenceRoutes(route, signals) {
  return {
    ...
    [signals.svgParserEvidence]: route('prove-svg-parser-source-evidence', 'layout-svg-parser', 'supply-svg-source-span-attribute-span-and-trivia-evidence'),
    [signals.svgReferenceGraphEvidence]: route('prove-svg-reference-graph-evidence', 'layout-svg-graph', 'supply-svg-id-href-and-url-reference-graph-evidence'),
    ...
    [signals.svgIdentityEvidence]: route('prove-svg-identity-evidence', 'layout-svg-identity', 'supply-stable-svg-id-reference-and-structural-addressability-evidence'),
    ...
    [signals.svgStructuralMerge]: route('admit-svg-structural-merge', 'layout-svg-graph', 'prove-svg-parser-identity-reference-and-runtime-boundary-evidence-or-fix-duplicate-identity'),
    ...
    [signals.svgBrowserRuntimeProof]: route('prove-svg-browser-runtime', 'browser-proof', 'produce-svg-dom-style-layout-accessibility-and-paint-proof-bundle'),
...
function htmlCssProjectMergeAdmissionMatrixRows(matrixRow, signals) {
  return [
    ...
    matrixRow('svg-parser-source-evidence', 'bounded-evidence', ['svg-parser-source-evidence'], [signals.svgParserEvidence]),
    matrixRow('svg-reference-graph-evidence', 'bounded-evidence', ['svg-reference-graph'], [signals.svgReferenceGraphEvidence]),
    matrixRow('html-identity-evidence', 'bounded-evidence', ['html-identity-evidence'], [signals.htmlIdentityEvidence]),
    matrixRow('svg-identity-evidence', 'bounded-evidence', ['svg-identity-evidence'], [signals.svgIdentityEvidence]),
    ...
    matrixRow('svg-structural-merge-admission', 'partial', ['svg-structural-merge'], [signals.svgStructuralMerge]),
    ...
    matrixRow('svg-browser-runtime-proof', 'bounded-evidence', ['svg-browser-runtime-proof'], [signals.svgBrowserRuntimeProof]),
...
function htmlCssProjectMergeMissingEvidenceItems(summary, signals, missingEvidenceItem) {
  ...
  if (summary.svgFiles && summary.svgParserEvidenceFiles !== summary.svgFiles) items.push(missingEvidenceItem({ code: signals.svgParserEvidence, scope: 'layout-svg-parser', kind: 'svg-parser-source-evidence', proofLevel: 'svg-parser-source-evidence', action: 'review', summary: `SVG project merge has parser/source evidence for ${summary.svgParserEvidenceFiles}/${summary.svgFiles} file(s); require one root <svg>, balanced XML-style tags, source-bound element spans, attribute spans, and zero parser evidence failures before SVG parser admission.` }));
  if (summary.svgFiles && summary.svgReferenceGraphEvidenceFiles !== summary.svgFiles) items.push(missingEvidenceItem({ code: signals.svgReferenceGraphEvidence, scope: 'layout-svg-graph', kind: 'svg-reference-graph-evidence', proofLevel: 'svg-reference-graph', action: 'review', summary: `SVG project merge has reference graph evidence for ${summary.svgReferenceGraphEvidenceFiles}/${summary.svgFiles} file(s) with ${summary.svgReferenceGraphMissingReferenceRecords ?? 0} missing local reference record(s); require source-bound id definitions plus href/xlink:href/url(#id) reference evidence before SVG structural admission.`, suggestedInput: { svgReferenceGraphEvidence: true, svgUniqueReferenceTargets: true } }));
  ...
  if (summary.svgFiles && summary.svgIdentityEvidenceFiles !== summary.svgFiles) items.push(missingEvidenceItem({ code: signals.svgIdentityEvidence, scope: 'layout-svg-identity', kind: 'svg-identity-evidence', proofLevel: 'svg-identity-evidence', action: 'review', summary: `SVG project merge has structural identity evidence for ${summary.svgIdentityEvidenceFiles}/${summary.svgFiles} file(s) with ${summary.svgDuplicateIdentityEvidenceFiles ?? 0} duplicate identity evidence file(s); require unique id/data-frontier-key identity, stable structural addressability, and explicit reference-boundary review for defs/use/clipPath/mask/filter/gradient ids before SVG structural admission.` }));
  ...
  if (summary.svgFiles && svgStructuralMergePrerequisitesStatus(summary) !== 'passed') items.push(missingEvidenceItem({ code: signals.svgStructuralMerge, scope: 'layout-svg-graph', kind: 'svg-structural-merge-proof', proofLevel: 'svg-structural-merge', action: 'review', summary: `SVG structural admission has ${summary.svgMergedFiles ?? 0}/${summary.svgFiles} merged file(s), ${summary.svgBlockedFiles ?? 0} blocked file(s), parser evidence for ${summary.svgParserEvidenceFiles ?? 0}/${summary.svgFiles} file(s), and identity evidence for ${summary.svgIdentityEvidenceFiles ?? 0}/${summary.svgFiles} file(s); require every SVG file to merge with SVG parser/source evidence and stable identity evidence before structural admission. Browser paint, layout, focus, accessibility, and event behavior remain separate proof rows.`, suggestedInput: { svgRuntimeProofsByPath: true, svgUniqueExplicitIdentityKeys: true, svgParserSourceEvidence: true, svgIdentityEvidence: true, svgBrowserRuntimeProof: true } }));
  ...
  if (summary.svgMergedFiles && !summary.svgBrowserRuntimeProofs) items.push(missingEvidenceItem({ code: signals.svgBrowserRuntimeProof, scope: 'browser-proof', kind: 'svg-browser-runtime-proof', proofLevel: 'svg-browser-runtime-proof', action: 'review', summary: 'SVG structural source merge was available, but runtime-evidence-bound SVG browser proof was not attached; produce a source-bound proof bundle with command, probe id, evidence hash, DOM snapshot hash, computed style hash, bounding-box/layout hash, accessibility/title/desc snapshot hash, focus/event trace where relevant, paint/screenshot hash when visual output changes, and no broad proof self-claims.', suggestedInput: { svgBrowserRuntimeProof: true, svgRuntimeProofsByPath: true, playwrightAssertionRuntimeProof: true } }));
...
function htmlCssProjectMergeMatrixProofStatus(level, summary) {
  ...
  if (level === 'svg-parser-source-evidence') return summary.svgFiles ? (summary.svgParserEvidenceFailedFiles ? 'failed' : summary.svgParserEvidenceFiles === summary.svgFiles ? 'passed' : 'missing') : 'absent';
  if (level === 'svg-reference-graph') return summary.svgFiles ? (summary.svgReferenceGraphEvidenceFailedFiles ? 'failed' : summary.svgReferenceGraphEvidenceFiles === summary.svgFiles ? 'passed' : 'missing') : 'absent';
  ...
  if (level === 'svg-identity-evidence') return summary.svgFiles ? (summary.svgIdentityEvidenceFailedFiles ? 'failed' : summary.svgIdentityEvidenceFiles === summary.svgFiles ? 'passed' : 'missing') : 'absent';
  ...
  if (level === 'svg-structural-merge') return svgStructuralMergePrerequisitesStatus(summary);
  ...
  if (level === 'svg-browser-runtime-proof') return summary.svgFiles ? (summary.svgBrowserRuntimeProofBlockedFiles ? 'failed' : summary.svgBrowserRuntimeProofs ? 'passed' : 'missing') : 'absent';
  if (level === 'browser-runtime-proof') return summary.htmlCssFiles ? (summary.htmlCssBrowserRuntimeProofBlockedFiles ? 'failed' : summary.htmlCssBrowserRuntimeProofs ? 'passed' : 'missing') : 'absent';
  return undefined;
}
...
function svgStructuralMergePrerequisitesStatus(summary) {
