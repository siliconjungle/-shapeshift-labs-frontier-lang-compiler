...
export interface UniversalTranslationConstraintSummary { readonly id?: string; readonly status?: string; readonly action?: string; readonly requiredKinds: RO<string>; readonly representedKinds: RO<string>; readonly missingKinds: RO<string>; readonly missingEvidence: RO<string>; } type TCS = UniversalTranslationConstraintSummary; type RO<T> = readonly T[]; type Rdy = SMR|string; type TCN = 'lifetimeConstraint'|'controlFlowConstraint'|'borrowScopeConstraint'|'borrowCheckerConstraint'|'dataLayoutConstraint'|'effectConstraint'|'concurrencyModelConstraint'|'errorModelConstraint'|'evaluationModelConstraint'|'hostEnvironmentConstraint'|'memoryModelConstraint'|'metaprogrammingConstraint'|'scopeBindingConstraint'|'moduleConstraint'|'objectModelConstraint'|'protocolConstraint'|'typeConstraint'; type TCF = { readonly [K in TCN]?: TCS } & { readonly [K in `${TCN}Status`]?: string } & { readonly [K in `${TCN}Action`]?: string } & { readonly [K in `${TCN}MissingEvidence`]: RO<string> };
...
export interface UniversalConversionPlanQuery extends URCQ, UniversalInterlinguaQuery, ResQ, LtQ, CfQ, BsQ, BcQ, DlQ, EfQ, ConQ, ErrQ, EvalQ, HostQ, MemQ, MetaQ, SbQ, ModQ, ObjQ, ProtoQ, TyQ {
  readonly sourceLanguage?: SL | string;
