...
owned-value creation, shared-borrow compatibility obligations, mutable-borrow
alias/loan exclusion obligations, possible lexical moves, call-argument ownership transfers,
...
`impl Trait`, and type-lifetime obligations become universal type constraints,
while trait/protocol/interface contracts also become protocol constraints with
required members, associated types, bounds, implementation/coherence rules,
object safety, and dispatch obligations for translation admission.
...
| SVG parser/source evidence | bounded-evidence | SVG project files are counted separately through `svgFiles`, `svgMergedFiles`, `svgBlockedFiles`, SVG parser evidence counters, and `svg-parser-source-evidence` matrix proof statuses. Current evidence is source-span and attribute-span bounded around a standalone XML-shaped SVG lexical scanner with exact `base` / `worker` / `head` side hashes, one-root `<svg>` validation, balanced tag validation, and fail-closed parser evidence failures; it does not claim paint, layout, accessibility, focus, event, or browser runtime equivalence. |
| SVG reference graph evidence | bounded-evidence | SVG reference proof uses `svgReferenceGraphEvidenceFiles`, definition/reference counters, missing-reference counters, and the `svg-reference-graph-evidence` proof status. Current evidence records source-bound local `id` definitions, `href` / `xlink:href` references, and `url(#id)` paint/resource references for gradients, masks, filters, markers, clip paths, symbols, and use sites. Missing or duplicate local targets fail closed; this row does not claim paint, layout, animation, focus, accessibility, event, or browser runtime equivalence. |
...
| SVG structural merge admission | partial | SVG structural admission uses `svg-structural-merge` proof statuses plus SVG parser and identity evidence. It can admit bounded standalone SVG source merges when the markup engine merges cleanly and required SVG parser/identity evidence is present, while duplicate identity, malformed XML-shaped SVG, reference-sensitive paint/server changes, and runtime-sensitive behavior stay fail-closed. SVG paint, layout, focus, accessibility, event, animation, and screenshot equivalence are not inferred from structural source evidence; missing proof routes to `admit-svg-structural-merge` and runtime proof remains a separate row. |
| CSS cascade merge admission | partial | CSS cascade admission uses `css-cascade-merge` proof statuses plus parser, selector, ordered duplicate occurrence, shape-keyed scoped cascade, dependency, and CSS Module use-site evidence. Stable repeated same-property declarations can now emit `cssOrderedCascadeOccurrenceEvidenceFiles` / `cssOrderedCascadeOccurrenceEvidenceRecords` and admit disjoint occurrence-indexed edits; duplicate count/order/shape changes and same-occurrence parallel edits still fail closed. Parser-backed nested selector expansion now emits source-bound declaration spans for nested scoped rules, rejects stale expansion proofs, and still requires source-bound scoped cascade proof before admission. Dependency proof, generated class maps, bundler transform identity, source-map proof, and dynamic use sites stay fail-closed when absent. CSS Module transform gaps are exposed by separate rows below; none of these rows claims browser, render, or cascade equivalence. |
...
- HTML structural merge admission now carries generic `htmlTokenListMergeEvidence` for parser-backed `class`, `part`, and `itemprop` token-list additions/removals plus `htmlUnkeyedStructuralAddEvidence` / `htmlUnkeyedStructuralDeleteEvidence` / `htmlUnkeyedStructuralMoveEvidence` for bounded unkeyed child subtree add, delete, or exact keyed-sibling-anchored move evidence under explicit parents; class remains backward-compatible through `htmlClassTokenMergeEvidence`, while duplicate tokens, order-only ambiguity, unsupported token-like attributes, general or ambiguous unkeyed reorders, unkeyed structural edits under unkeyed parents, same-parent structural races, runtime-boundary attributes, and broad browser/render equivalence claims stay fail-closed.
- HTML/CSS browser runtime proof admission now accepts both the HTML package source-bound browser/runtime proof aliases and canonical `@shapeshift-labs/frontier-runtime-proof` source-bound capsules at project level. Canonical capsule proofs are bridged into the HTML adapter shape while preserving the original proof kind in admitted evidence, require exact source/output hashes, runtime command, probe id, evidence hash, required runtime signals, telemetry hash, and reject blocked environments, stale source hashes, broad self-claims, and excessive cumulative layout shift.
...
- CSS Modules graph records now infer local class export evidence, source-local composition graph hashes, export-only ICSS graph hashes, project-source graph hashes for cross-file `composes` / ICSS imports when every referenced `.module.css` source is present and source-inferred, source-bounded class helper token graph hashes for single top-level `className` helper calls whose CSS Module reads are statically enumerable, and source-bound finite dynamic key domains for bracket reads proven by local literal-union annotations, object-property type annotations, or local const object-literal variants; package/bundler-only composition, unresolved ICSS imports, generated class-name maps, bundler transform identity, source-map proof, unbounded dynamic member access, writes, nested helper calls, and arbitrary helper runtime equivalence remain fail-closed without host evidence.
- CSS Modules use-site graph evidence now suppresses `css-module-helper-call-unproved` only for source-bounded helper calls with deterministic token enumeration and suppresses duplicate JSX dynamic-key blockers only when the lexical graph has an exact `bounded-dynamic-bracket` proof for the same expression, while preserving transform proof blockers and keeping unbounded dynamic keys on review routes.
...
const packageGaps = queryUniversalCapabilityMatrix(universalMatrix, {
  packageSourceImporterOnly: true,
  packageTargetProjectionMissing: true
});
console.log(packageGaps.summary.languages); // source-importer packages that still need target projection work
...
const adtPatternConstraints = createUniversalAdtPatternConstraintEvidence({
  ...
  sourceAdtPatternRecords: [{
    kind: 'sum type enum variant payload tagged switch match exhaustive guard default',
    name: 'Result',
    variantNames: ['Ok', 'Err'],
  ...
  targetAdtPatternRecords: [{ kind: 'tagged union variant', name: 'Result' }]
});
console.log(adtPatternConstraints.missingKinds); // payload, tag, match, guard, or exhaustiveness obligations needing target evidence
console.log(adtPatternConstraints.claims.patternExhaustivenessClaim); // false without source-bound exhaustiveness proof
...
const dataLayoutConstraints = createUniversalDataLayoutConstraintEvidence({
  sourceLanguage: 'c',
  target: 'rust',
  sourceDataLayoutRecords: [
    { kind: 'packed-layout field-order field-offset bitfield-layout', structId: 'Packet', offsetBytes: 4 },
    { kind: 'ffi-boundary abi-calling-convention endian pointer-width', callingConvention: 'cdecl' }
  ],
  targetDataLayoutRecords: [{ kind: 'repr(C) field-order alignment', structId: 'Packet' }]
});
console.log(dataLayoutConstraints.missingKinds); // layout/ABI obligations still needing target evidence
console.log(dataLayoutConstraints.claims.dataLayoutEquivalenceClaim); // false without representation proof
...
const protocolConstraints = createUniversalProtocolConstraintEvidence({
  sourceLanguage: 'rust',
  target: 'typescript',
  sourceProtocols: [{
  ...
  targetProtocols: [{ kind: 'interface structural', name: 'Cache', requirementNames: ['get'] }]
});
console.log(protocolConstraints.missingKinds); // trait/protocol/interface obligations still needing target evidence
console.log(protocolConstraints.claims.protocolEquivalenceClaim); // false without protocol-contract proof
...
const scopeBindingConstraints = createUniversalScopeBindingConstraintEvidence({
  sourceLanguage: 'typescript',
  target: 'rust',
  sourceScopeBindingRecords: [
    { kind: 'lexical scope binding-identity use-def reference', bindingId: 'readUser', referenceId: 'call_readUser' },
    { kind: 'closure capture mutable-capture receiver-binding type-value namespace', name: 'count' },
    { kind: 'dynamic-lookup late-binding', name: 'record[key]' }
  ]
});
console.log(scopeBindingConstraints.missingKinds); // use-def/name-resolution/closure obligations still needing target evidence
console.log(scopeBindingConstraints.claims.nameResolutionEquivalenceClaim); // false without scope/use-def proof
...
const objectModelConstraints = createUniversalObjectModelConstraintEvidence({
  sourceLanguage: 'javascript',
  target: 'rust',
  sourceObjectModelRecords: [
    { kind: 'prototype-chain object-identity reference-semantics', prototypeId: 'AdminUser.prototype' },
    { kind: 'virtual-dispatch override-rules super-dispatch', methodId: 'AdminUser.save' }
  ],
  targetObjectModelRecords: [{ kind: 'class-construction static-dispatch', classId: 'AdminUser' }]
});
console.log(objectModelConstraints.missingKinds); // prototype/dispatch/object-identity obligations still needing target evidence
console.log(objectModelConstraints.claims.objectModelEquivalenceClaim); // false without source-bound object-model proof
...
console.log(conversionArtifacts.evidenceReceipts[0].proofEvidenceIds); // route-bound proof IDs in the artifact bundle
console.log(conversionArtifacts.routeArtifacts[0].translationAdmission.action); // review/materialization task
console.log(conversionArtifacts.routeArtifacts[0].evidenceReceipt.missingEvidence); // receipt-level missing proof/source evidence
...
console.log(conversionArtifacts.index.semanticOperationInterlinguaLoweringDispositions); // lowering labels on operation metadata
...
record. The same composed obligation is carried through `translationAdmission`,
the interlingua `borrow-checker` constraint family, artifact indexes, and
`collect-interlingua-obligation-proof` worklist items so coordinators can route
borrow-checker proof gaps as one first-class review surface. It still keeps
`borrowCheckerClaim`, `semanticEquivalenceClaim`, and `autoMergeClaim` false.
...
match/switch dispatch, exhaustiveness, guards, wildcard/default arms,
fallthrough, refutability, recursive variants, option/result conventions,
nullability encoding, case ordering, generics, and representation layout.
ADT/pattern queries such as
`adtPatternConstraintMissingKind: "exhaustiveness"` or
`interlinguaConstraintFamily: "adt-pattern"` let a coordinator distinguish
"target code has a union-like type" from "target lowering preserved variant
payloads and match coverage." The record remains conservative: ADT-equivalence,
pattern-exhaustiveness, variant-payload-equivalence, semantic-equivalence, and
...
collector can ask for missing `adt-pattern`/`exhaustiveness` receipt proof
without loading every materialized route artifact.
...
isolation, channel communication, cancellation propagation, task lifecycle,
backpressure, reentrancy, and task-local context. Concurrency-model queries such
as `concurrencyModelConstraintMissingKind: "cancellation-propagation"` or
`interlinguaConstraintFamily: "concurrency-model"` let a coordinator distinguish
"target code exists" from "target lowering preserved when work is scheduled,
cancelled, joined, or isolated." The record remains conservative:
concurrency-model equivalence, scheduler-equivalence, semantic-equivalence, and
auto-merge claims stay false unless separate proof is attached.

Routes can also carry `errorModelConstraint`, a throw/result/panic/recovery
admission record for checked and unchecked exceptions, fallible result returns,
panic unwind or abort behavior, catch/recovery handlers, cleanup/finally/defer
boundaries, error boundaries, cancellation, and traps. Error-model queries such
as `errorModelConstraintMissingKind: "cleanup-boundary"` or
`interlinguaConstraintFamily: "error-model"` let a coordinator distinguish
"target code exists" from "target lowering preserved how failures travel and
which cleanup runs." The record remains conservative: error-model equivalence,
semantic-equivalence, and auto-merge claims stay false unless separate proof is
attached.

Routes can also carry `evaluationModelConstraint`, an expression/value-semantics
admission record for operand evaluation order, side-effect ordering,
short-circuiting, lazy or eager evaluation, truthiness, nullish semantics,
equality and comparison, implicit and numeric coercion, integer overflow,
integer division, floating-point behavior, operator overloads, property access,
indexing bounds, destructuring defaults, pattern binding, and receiver binding.
Evaluation-model queries such as
`evaluationModelConstraintMissingKind: "integer-division"` or
`interlinguaConstraintFamily: "evaluation-model"` let a coordinator distinguish
"target code exists" from "target lowering preserved what the expression means
when it executes." The record remains conservative: evaluation-model
equivalence, value-semantics equivalence, semantic-equivalence, and auto-merge
claims stay false unless separate proof is attached.

Routes can also carry `numericSemanticsConstraint`, a focused arithmetic/value
domain record for numeric type identity, integer width, signedness, overflow,
division and remainder behavior, floating-point format, rounding, NaN/infinity
and signed-zero behavior, numeric coercion, BigInt/decimal semantics, literal
parsing, bitwise operators, and numeric ordering. Numeric-semantics queries such
as `numericSemanticsConstraintMissingKind: "overflow-behavior"` or
`interlinguaConstraintFamily: "numeric-semantics"` let a coordinator distinguish
"the target can emit arithmetic" from "the target preserved the source numeric
contract." The record remains conservative: numeric-equivalence,
arithmetic-equivalence, floating-point-equivalence, semantic-equivalence, and
auto-merge claims stay false unless separate proof is attached.

Routes can also carry `textSemanticsConstraint`, a focused string and Unicode
record for text type identity, encoding, code units, code points, grapheme
clusters, indexing and slicing units, normalization, locale, collation, case
mapping, regex behavior, escaping, interpolation, null termination, byte/text
boundaries, and string mutability. Text-semantics queries such as
`textSemanticsConstraintMissingKind: "normalization"` or
`interlinguaConstraintFamily: "text-semantics"` let a coordinator distinguish
"the target can emit string code" from "the target preserved the source text
contract." The record remains conservative: text-equivalence,
Unicode-equivalence, regex-equivalence, semantic-equivalence, and auto-merge
claims stay false unless separate proof is attached.

Routes can also carry `collectionSemanticsConstraint`, a focused container
record for array/list/vector/slice/map/set/iterator identity, order, indexing,
bounds, length, slicing, sparse-hole behavior, duplicate policy, map keys, set
membership, equality, hashing, comparators, mutability, persistence,
copy-on-write, iterator invalidation, lazy traversal, capacity growth, and
concurrency safety. Collection-semantics queries such as
`collectionSemanticsConstraintMissingKind: "key-equality"` or
`interlinguaConstraintFamily: "collection-semantics"` let a coordinator
distinguish "the target has a collection API" from "the target preserved the
source collection contract." The record remains conservative:
collection-equivalence, ordering-equivalence, lookup-equivalence,
semantic-equivalence, and auto-merge claims stay false unless separate proof is
attached.

Routes can also carry `serializationSemanticsConstraint`, a focused wire-format
record for serialization format, binary/text wire shape, field naming and order,
omitted/default/null/unknown field behavior, enum tags, endianness, alignment,
varints, schema versioning, canonical output, deterministic encoding, precision
loss, roundtrip stability, validation, escaping/security, streaming/framing, and
codec runtime dependencies. Serialization-semantics queries such as
`serializationSemanticsConstraintMissingKind: "canonicalization"` or
`interlinguaConstraintFamily: "serialization-semantics"` let a coordinator
distinguish "the target can emit a type" from "the target preserved the source
wire contract." The record remains conservative: serialization-equivalence,
wire-equivalence, roundtrip-equivalence, semantic-equivalence, and auto-merge
claims stay false unless separate proof is attached.

Routes can also carry `dependencySemanticsConstraint`, a focused dependency
graph record for package managers, manifest schemas, version ranges, resolved
versions, lockfile integrity, dependency classes, peer/optional/dev
dependencies, feature flags, workspace boundaries, registry sources, lifecycle
scripts, native ABI, build tools, package-manager versions, frozen/offline cache
policy, hoist/dedupe layout, provenance, and supply-chain trust.
Dependency-semantics queries such as
`dependencySemanticsConstraintMissingKind: "lockfile-integrity"` or
`interlinguaConstraintFamily: "dependency-semantics"` let a coordinator
distinguish "the target imports a package" from "the target preserved the source
dependency contract." The record remains conservative: dependency-equivalence,
resolution-equivalence, lockfile-equivalence, supply-chain-equivalence,
semantic-equivalence, and auto-merge claims stay false unless separate proof is
attached.

Routes can also carry `memoryModelConstraint`, a synchronization and shared-memory
admission record for atomics, memory ordering, happens-before edges, lock
discipline, data-race-freedom obligations, shared memory, volatile access,
thread/worker boundaries, channels, actor isolation, and reentrancy. Memory-model
queries such as `memoryModelConstraintMissingKind: "atomic-ordering"` or
`interlinguaConstraintFamily: "memory-model"` let a coordinator distinguish
"target code exists" from "target lowering preserved the source synchronization
contract." The record remains conservative: memory-model equivalence,
...
Routes can also carry `scopeBindingConstraint`, a scope and name-resolution
admission record for lexical/block/function/module scopes, binding identity,
use-def/reference edges, shadowing, hoisting, temporal dead zones, closure and
mutable captures, receiver/`super` binding, dynamic and late lookup,
destructuring/pattern bindings, implicit globals, type/value namespaces, private
names, and labels. Scope-binding queries such as
`scopeBindingConstraintMissingKind: "closure-capture"` or
`interlinguaConstraintFamily: "scope-binding"` let a coordinator distinguish
"target code contains a name" from "target lowering preserved which declaration
that name resolves to." The record remains conservative: name-resolution,
closure, scope, semantic-equivalence, and auto-merge claims stay false unless
separate proof is attached.
...
Routes can also carry `objectModelConstraint`, a class/prototype/object-dispatch
admission record for class construction, constructor order, field initialization,
object identity, reference vs value semantics, prototype chains, inheritance,
multiple inheritance, method-resolution order, virtual/static dispatch, override
rules, `super` dispatch, traits, mixins, extension methods, visibility, sealed or
final classes, destructors/finalizers, reflection, and metaclasses. Object-model
queries such as `objectModelConstraintMissingKind: "virtual-dispatch"` or
`interlinguaConstraintFamily: "object-model"` let a coordinator distinguish
"target code has fields and methods" from "target lowering preserved how objects
are constructed, identified, inherited, and dispatched." The record remains
conservative: object-model equivalence, dispatch equivalence,
semantic-equivalence, and auto-merge claims stay false unless separate proof is
attached.

Routes can also carry `dataLayoutConstraint`, a representation and ABI admission
record for type size, alignment, field order, field offsets, padding, packed
layout, bitfields, unions, enum discriminants, tagged unions, endianness, integer
width, floating-point format, pointer width, calling convention, FFI boundaries,
serialization layout, nullable representation, niche optimization, representation
attributes, and layout stability. Data-layout queries such as
`dataLayoutConstraintMissingKind: "field-offset"` or
`interlinguaConstraintFamily: "data-layout"` let a coordinator distinguish
"target code has a matching type" from "target lowering preserved how bytes are
laid out or passed across a runtime boundary." The record remains conservative:
data-layout equivalence, ABI equivalence, semantic-equivalence, and auto-merge
claims stay false unless separate proof is attached.

Routes can also carry `protocolConstraint`, a trait/interface/protocol admission
record for required members, associated types, generic parameters,
trait/protocol/concept bounds, implementations, blanket or conditional impls,
default methods, extension methods, static and dynamic dispatch, object safety,
coherence and orphan rules, sealed protocols, marker/auto traits,
existential/opaque protocol values, nominal-vs-structural contracts,
higher-ranked bounds, and specialization. Protocol queries such as
`protocolConstraintMissingKind: "associated-type"` or
`interlinguaConstraintFamily: "protocol"` let a coordinator distinguish "target
types are shaped similarly" from "target lowering preserved the behavioral
contract that values must satisfy." The record remains conservative:
protocol-equivalence, implementation-coherence, dispatch-equivalence,
semantic-equivalence, and auto-merge claims stay false unless separate proof is
attached.
