...
- HTML structural merge admission now carries generic `htmlTokenListMergeEvidence` for parser-backed `class`, `part`, and `itemprop` token-list additions/removals; class remains backward-compatible through `htmlClassTokenMergeEvidence`, while duplicate tokens, order-only ambiguity, unsupported token-like attributes, runtime-boundary attributes, and broad browser/render equivalence claims stay fail-closed.
