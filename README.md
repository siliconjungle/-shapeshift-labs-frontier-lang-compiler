...
| CSS selector target evidence | bounded-evidence | CSS selector target proof uses `cssSelectorTargetEvidenceFiles`, selector target graph/specificity/move counters, selector conflict/rebase counters, and the `css-selector-target-evidence` proof status. Target evidence remains bounded to parser-backed selector/target/rebase facts; selector-target rebases involving selector-list functional pseudos now fail closed with `css-selector-functional-pseudo-specificity-unproved` until @shapeshift-labs/frontier-lang-css emits exact parser-backed Selectors Level 4 specificity records. This row does not claim cascade or browser runtime equivalence. |
