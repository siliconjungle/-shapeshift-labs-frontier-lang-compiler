...
- HTML structural merge admission now carries generic `htmlTokenListMergeEvidence` for parser-backed `class`, `part`, and `itemprop` token-list additions/removals plus `htmlUnkeyedStructuralAddEvidence` for add-only unkeyed child subtrees under explicit parents; class remains backward-compatible through `htmlClassTokenMergeEvidence`, while duplicate tokens, order-only ambiguity, unsupported token-like attributes, unkeyed deletes/moves/reorders, same-parent structural races, runtime-boundary attributes, and broad browser/render equivalence claims stay fail-closed.
