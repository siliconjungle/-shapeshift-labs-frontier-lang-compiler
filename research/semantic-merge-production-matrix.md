...
| CSS dependencies and runtime descriptors | partial | Custom-property proofs, keyframes plus animation-name auto-proofs, font/url/runtime descriptor blockers, and descriptor evidence exist. | Extend non-custom-property auto-proofs to @font-face references, URL assets, and runtime descriptor families. |
...
| P2 | Font/url dependency auto-proofs | CSS dependency auto-admission now covers custom properties and keyframes, but font-face and URL dependency families remain review-heavy. | Add source-bound `@font-face` plus `font-family` rename and URL asset rewrite fixtures with stale-proof rejection. |
