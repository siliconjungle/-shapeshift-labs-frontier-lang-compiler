...
| JSX/TSX child order and render layout | partial | Child-order helper coverage exists and aggregate conflict wiring is now covered by focused smoke. Render return, keyed collection, branch, generic branch evidence, and fragment fixtures exist. | Finish event-handler proof bridge, hook-dependency proof bridge, and context/wrapper render proofs. |
...
| CSS dependencies and runtime descriptors | partial | Custom-property, keyframes plus animation-name, font-face plus font-family, and URL asset auto-proofs exist; runtime descriptor blockers and descriptor evidence exist. | Extend source-bound proof bundles to CSS runtime descriptor families such as @property and @page. |
...
| P2 | CSS runtime descriptor proof bundles | CSS dependency auto-admission now covers custom properties, keyframes, font-face family renames, and URL assets, but descriptor families remain review-heavy. | Add source-bound @property and @page descriptor fixtures with stale-proof rejection and browser-runtime evidence requirements kept explicit. |
