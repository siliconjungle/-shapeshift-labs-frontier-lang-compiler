...
const universalCapabilityQuery: UniversalCapabilityMatrixQueryResult = queryUniversalCapabilityMatrix(createUniversalCapabilityMatrix({
  imports: [imported],
  targets: ['rust']
}), {
  packageTargetProjectionSupported: true
});
universalCapabilityQuery.summary.languages satisfies readonly string[];
universalCapabilityQuery.bestRow?.packageContract.releaseReady satisfies boolean | undefined;
...
async function projectAdmissionTypes(): Promise<void> {
  ...
  const projectMetadata: NativeProjectImportResultMetadata | undefined = projectResult.metadata;
  const projectSourcePreservationSummary: NativeProjectSourcePreservationSummary | undefined = projectMetadata?.sourcePreservationSummary;
  const projectRegionSummary: NativeImportRegionSummary | undefined = projectMetadata?.regionSummary;
  const projectSourceMapSummary: NativeImportSourceMapSummary | undefined = projectMetadata?.sourceMapSummary;
  const projectAdapterCoverageSummary: NativeImportAdapterCoverageContract | undefined = projectMetadata?.adapterCoverageSummary;
  ...
  projectMetadata?.sourceCount satisfies number | undefined;
  projectMetadata?.sourcePaths satisfies readonly string[] | undefined;
  projectSourcePreservationSummary?.total satisfies number | undefined;
  projectSourcePreservationSummary?.ids satisfies readonly string[] | undefined;
  projectRegionSummary?.total satisfies number | undefined;
  projectSourceMapSummary?.mappingCount satisfies number | undefined;
  projectAdapterCoverageSummary?.total satisfies number | undefined;
}
...
const conversionWorklistTypes = createUniversalConversionWorklist(conversionPlanTypes, {
  target: 'javascript'
});
conversionWorklistTypes.summary.byKind satisfies Readonly<Record<string, number>>;
conversionWorklistTypes.metadata.autoMergeClaim satisfies false;
UniversalConversionWorkItemKinds.includes('collect-translation-proof');
const conversionWorklistQueryTypes = queryUniversalConversionWorklist(conversionWorklistTypes, {
  target: 'javascript'
});
conversionWorklistQueryTypes.bestItem?.semanticEquivalenceClaim satisfies false | undefined;
...
const impactSummary: SemanticImportImpactSummary = sidecar.semanticImpact;
const sourcePreservationCount: number = sidecar.summary.sourcePreservationRecords + (sourcePreservationRow?.lossIds.length ?? 0) + proofSpecSummary.obligations + paradigmSummary.loweringRecords + impactSummary.summary.total;
