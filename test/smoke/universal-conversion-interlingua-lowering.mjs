...
const manualObligationRecord = createUniversalInterlinguaRecord({ route: { id: 'manual_obligation_route', sourceLanguage: 'javascript', target: 'rust', mode: 'target-adapter', readiness: 'needs-evidence', borrowScopeConstraint: { id: 'manual_borrow_scope', status: 'needs-evidence', missingKinds: ['borrow-across-await'], evidenceIds: ['manual_constraint_edge_proof'], constraints: [{ kind: 'borrow-across-await', status: 'missing', evidenceIds: ['manual_obligation_proof'] }] } } }); assert.equal(interlinguaRecordMatches(manualObligationRecord, { interlinguaConstraintEvidenceId: 'manual_constraint_edge_proof' }), true); assert.equal(interlinguaRecordMatches(manualObligationRecord, { interlinguaConstraintObligationEvidenceId: 'manual_obligation_proof' }), true); assert.equal(interlinguaRecordMatches(manualObligationRecord, { interlinguaConstraintObligationEvidenceId: 'manual_constraint_edge_proof' }), false);
