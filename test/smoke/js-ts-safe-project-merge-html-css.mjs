...
function scopedProof({ id, sourcePath, graphHash, base, worker, head, output, scope, shapeKey, cascadeKeys, properties, sides }) { return { id, kind: 'css-source-bound-scoped-cascade-proof', status: 'passed', sourcePath, reasonCode: 'css-scoped-cascade-equivalence-unproved', sides, selectors: ['.button'], scopes: [scope], cascadeKeys, properties, scopedCascadeGraphHash: graphHash, ...(shapeKey ? { scopedCascadeGraphShapeKey: shapeKey } : {}), baseSourceHash: hashSemanticValue(base), workerSourceHash: hashSemanticValue(worker), headSourceHash: hashSemanticValue(head), outputSourceHash: hashSemanticValue(output) }; }
