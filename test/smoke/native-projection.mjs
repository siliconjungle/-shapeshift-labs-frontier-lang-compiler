...
assert.equal(preservedNativeProjection.metadata.projectionReview.status, 'preserved-source');
assert.equal(preservedNativeProjection.metadata.projectionReview.reviewRequired, false);
assert.equal(preservedNativeProjection.evidence[0].metadata.projectionReview.status, 'preserved-source');
...
assert.equal(sameLanguageNativeCompile.metadata.projectionReview.status, 'preserved-source');
assert.equal(sameLanguageNativeCompile.metadata.projectionReview.reviewRequired, true);
assert.equal(sameLanguageNativeCompile.sourceMap.metadata.projectionReview.fallbackReasons.includes('target-losses'), true);
...
assert.equal(rustNativeCompileBlocked.metadata.projectionReview.status, 'blocked');
assert.equal(rustNativeCompileBlocked.metadata.projectionReview.fallbackReasons.includes('missing-target-adapter'), true);
assert.equal(rustNativeCompileBlocked.sourceMap.metadata.projectionReview.targetLossClass, 'missingAdapter');
...
assert.equal(rustNativeCompileWithAdapter.metadata.projectionReview.status, 'target-adapter');
assert.equal(rustNativeCompileWithAdapter.metadata.projectionReview.targetProjectionAdapterId, 'fixture-js-to-rust-target-adapter');
